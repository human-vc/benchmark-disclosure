"""The discontinuity estimator has to find a jump that is there, and none that is not."""
...
def synthetic(jump=0.0, n_releases=90, seed=0, slope=0.00002):
    """Releases meeting benchmarks of varying vintage, with an optional jump at zero.

    Dense on purpose: the running variable is a difference of two date grids, so
    a coarse grid leaves few distinct maturities near the cutoff and a local
    linear fit will find jumps that are not there.
    """
...
def test_randomization_p_is_large_under_no_effect():
    """Flat inside the window, which is what local randomization assumes."""
...
def test_randomization_picks_up_a_slope_inside_the_window():
    """The assumption is no relationship with the running variable in the window.

    A trend there is indistinguishable from a jump under this estimator, which
    is why the window is chosen on balance and why the continuity framework is
    reported beside it.
    """
...
def test_window_selection_stops_at_the_first_imbalance():
    """It must not reopen a window after balance fails, or it is not nested."""
    frame = boundary.cells(synthetic())
