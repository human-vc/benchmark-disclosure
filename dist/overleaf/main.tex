...
Everything so far concerns a percentile constructed here on a panel assembled here, so the effect may belong to the construction, not leaderboards in general. HELM Lite publishes its full accuracy table at every versioned release, archives earlier ones, and reports a pool-relative headline \citep{liang2022helm}, so the same evidence reads at 14 points in time while the pool grows.
