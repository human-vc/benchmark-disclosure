...
\begin{tikzpicture}
  \begin{axis}[causalre wide, height=2.6cm, xmin=-360, xmax=720, xtick={-360,0,360,720}, xticklabels={,,,}, ylabel={newer share}, ymin=0.25, ymax=0.95, ytick={0.3,0.5,0.7,0.9}, name=mech]
  ...
  \end{axis}
  \begin{axis}[causalre wide, height=2.6cm, xmin=-360, xmax=720, xtick={-360,0,360,720}, xlabel={benchmark maturity at release (days)}, ylabel={standing}, ymin=30, ymax=75, at={(mech.south west)}, anchor=north west, yshift=-12pt,
...
Selective withholding is a statement about results a provider held and did not print, and the disclosure label observes only the printing. Four cuts organise how far the record reaches toward possession. The direct cases first: no document in the record announces an evaluation and withholds its number, so category D is empty over the 295 cells where a run is documented, and the record contains no observed instance of a held result going unprinted. Second, the omission deficit stratified by evidence tier. Withholding predicts the deficit weakens, and eventually inverts, as possession evidence strengthens; composition predicts no gradient. The point estimates are $+18.9$ over 21 releases for dropped benchmarks, $+11.3$ over 51 for peer-reported omissions, and $+11.3$ over 69 for silent ones, so the strongest-evidence tier stands furthest above its placebo rather than below it. Third, a change-based drop test. The estimator above compares levels; a sharper question is whether a benchmark is dropped after the family's standing on it deteriorates, which differences out persistent salience and relevance. Over the 42 family transitions with a scored predecessor report, 33 dropped and 86 retained cells give a dropped-minus-retained change of $-3.7$ percentile points, the sign withholding predicts, at a count-preserving permutation $p = 0.16$ within transitions; on raw scores, kept where both sit on the unit interval, the gap is $-0.7$ points at $p = 0.71$. This sample is wider than the estimator's 16 drops because it conditions only on a scored predecessor report, not on the estimator's within-release retention floor. Fourth, substitutions. Requiring the reporting table to hold its size while one benchmark is dropped and another inserted, the case a fixed-table explanation cannot cover, leaves no qualifying case in the record. Nothing in the four detects withholding; the change-based sign is the only reading in its direction, and it sits inside its own null.
