...
In our setting an independent evaluator reports scores for cells that have been passed over by the provider, and each $\tau_b$ makes non-existence verifiable. Denote the available set and scored cells by $A_i = \{b : \tau_b < t_i\}$ and $O_i$; then $D_i \subseteq E_i \subseteq A_i$ and $\theta = \Pr\!\left(b \notin D_i \mid b \in O_i \cap A_i\right)$.
...
\end{tabular}
\caption{%
Placebo coefficient by conditioning set, percentile points, within release. Two-way provider-by-benchmark standard errors (45 by 61 clusters; provider-only in Appendix~\ref{sec:supporting}), $n = 2{,}831$. Absorbed is the fall in absolute value from row one; peer count suppresses; the last row has both.}
...
\begin{tikzpicture}
  \begin{axis}[causalre wide, height=2.35cm, xmin=0, xmax=1, xtick={0,0.5,1}, xlabel={share of window newer than the focal model}, ymin=20, ymax=80,
