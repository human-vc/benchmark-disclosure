...
def fig_helm():
    ...
    The claim is that published order moves, so the figure plots order. A
    levels plot hides it: the closest reversing pair separates by 0.042 at the
    first release and 0.017 at the last, which is invisible on a zero-to-one
    axis. Rank makes a reversal a crossing, which is the whole point of a
    leaderboard and the reason bump charts exist.

    Sparse enough to read: across fourteen releases only fourteen consecutive
    rank changes occur, nine models move at all, and no model moves more than
    two places. The control moves nothing.
    """
